// *** WARNING: this file was generated by pulumi-java-gen. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package com.pulumi.azurenative.networkfunction.inputs;

import com.pulumi.core.annotations.Import;
import java.lang.String;
import java.util.Objects;


public final class GetCollectorPolicyPlainArgs extends com.pulumi.resources.InvokeArgs {

    public static final GetCollectorPolicyPlainArgs Empty = new GetCollectorPolicyPlainArgs();

    /**
     * Azure Traffic Collector name
     * 
     */
    @Import(name="azureTrafficCollectorName", required=true)
    private String azureTrafficCollectorName;

    /**
     * @return Azure Traffic Collector name
     * 
     */
    public String azureTrafficCollectorName() {
        return this.azureTrafficCollectorName;
    }

    /**
     * Collector Policy Name
     * 
     */
    @Import(name="collectorPolicyName", required=true)
    private String collectorPolicyName;

    /**
     * @return Collector Policy Name
     * 
     */
    public String collectorPolicyName() {
        return this.collectorPolicyName;
    }

    /**
     * The name of the resource group.
     * 
     */
    @Import(name="resourceGroupName", required=true)
    private String resourceGroupName;

    /**
     * @return The name of the resource group.
     * 
     */
    public String resourceGroupName() {
        return this.resourceGroupName;
    }

    private GetCollectorPolicyPlainArgs() {}

    private GetCollectorPolicyPlainArgs(GetCollectorPolicyPlainArgs $) {
        this.azureTrafficCollectorName = $.azureTrafficCollectorName;
        this.collectorPolicyName = $.collectorPolicyName;
        this.resourceGroupName = $.resourceGroupName;
    }

    public static Builder builder() {
        return new Builder();
    }
    public static Builder builder(GetCollectorPolicyPlainArgs defaults) {
        return new Builder(defaults);
    }

    public static final class Builder {
        private GetCollectorPolicyPlainArgs $;

        public Builder() {
            $ = new GetCollectorPolicyPlainArgs();
        }

        public Builder(GetCollectorPolicyPlainArgs defaults) {
            $ = new GetCollectorPolicyPlainArgs(Objects.requireNonNull(defaults));
        }

        /**
         * @param azureTrafficCollectorName Azure Traffic Collector name
         * 
         * @return builder
         * 
         */
        public Builder azureTrafficCollectorName(String azureTrafficCollectorName) {
            $.azureTrafficCollectorName = azureTrafficCollectorName;
            return this;
        }

        /**
         * @param collectorPolicyName Collector Policy Name
         * 
         * @return builder
         * 
         */
        public Builder collectorPolicyName(String collectorPolicyName) {
            $.collectorPolicyName = collectorPolicyName;
            return this;
        }

        /**
         * @param resourceGroupName The name of the resource group.
         * 
         * @return builder
         * 
         */
        public Builder resourceGroupName(String resourceGroupName) {
            $.resourceGroupName = resourceGroupName;
            return this;
        }

        public GetCollectorPolicyPlainArgs build() {
            $.azureTrafficCollectorName = Objects.requireNonNull($.azureTrafficCollectorName, "expected parameter 'azureTrafficCollectorName' to be non-null");
            $.collectorPolicyName = Objects.requireNonNull($.collectorPolicyName, "expected parameter 'collectorPolicyName' to be non-null");
            $.resourceGroupName = Objects.requireNonNull($.resourceGroupName, "expected parameter 'resourceGroupName' to be non-null");
            return $;
        }
    }

}
