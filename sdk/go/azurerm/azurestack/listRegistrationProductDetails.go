// *** WARNING: this file was generated by the Pulumi SDK Generator. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

package azurestack

import (
	"github.com/pulumi/pulumi/sdk/v2/go/pulumi"
)

func ListRegistrationProductDetails(ctx *pulumi.Context, args *ListRegistrationProductDetailsArgs, opts ...pulumi.InvokeOption) (*ListRegistrationProductDetailsResult, error) {
	var rv ListRegistrationProductDetailsResult
	err := ctx.Invoke("azurerm:azurestack:listRegistrationProductDetails", args, &rv, opts...)
	if err != nil {
		return nil, err
	}
	return &rv, nil
}

type ListRegistrationProductDetailsArgs struct {
	// Name of the product.
	ProductName string `pulumi:"productName"`
	// Name of the Azure Stack registration.
	RegistrationName string `pulumi:"registrationName"`
	// Name of the resource group.
	ResourceGroup string `pulumi:"resourceGroup"`
}

// Extended description about the product required for installing it into Azure Stack.
type ListRegistrationProductDetailsResult struct {
	// The URI to the .azpkg file that provides information required for showing product in the gallery.
	GalleryPackageBlobSasUri string `pulumi:"galleryPackageBlobSasUri"`
	// Specifies the kind of the product (virtualMachine or virtualMachineExtension).
	ProductKind string `pulumi:"productKind"`
	// Specifies additional properties describing the product.
	Properties ExtendedProductPropertiesResponse `pulumi:"properties"`
}
