// *** WARNING: this file was generated by the Pulumi SDK Generator. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.AzureNative.ContainerInstance.V20211001.Inputs
{

    /// <summary>
    /// IP address for the container group.
    /// </summary>
    public sealed class IpAddressArgs : global::Pulumi.ResourceArgs
    {
        /// <summary>
        /// The Dns name label for the IP.
        /// </summary>
        [Input("dnsNameLabel")]
        public Input<string>? DnsNameLabel { get; set; }

        /// <summary>
        /// The value representing the security enum.
        /// </summary>
        [Input("dnsNameLabelReusePolicy")]
        public InputUnion<string, Pulumi.AzureNative.ContainerInstance.V20211001.AutoGeneratedDomainNameLabelScope>? DnsNameLabelReusePolicy { get; set; }

        /// <summary>
        /// The IP exposed to the public internet.
        /// </summary>
        [Input("ip")]
        public Input<string>? Ip { get; set; }

        [Input("ports", required: true)]
        private InputList<Inputs.PortArgs>? _ports;

        /// <summary>
        /// The list of ports exposed on the container group.
        /// </summary>
        public InputList<Inputs.PortArgs> Ports
        {
            get => _ports ?? (_ports = new InputList<Inputs.PortArgs>());
            set => _ports = value;
        }

        /// <summary>
        /// Specifies if the IP is exposed to the public internet or private VNET.
        /// </summary>
        [Input("type", required: true)]
        public InputUnion<string, Pulumi.AzureNative.ContainerInstance.V20211001.ContainerGroupIpAddressType> Type { get; set; } = null!;

        public IpAddressArgs()
        {
        }
        public static new IpAddressArgs Empty => new IpAddressArgs();
    }
}
