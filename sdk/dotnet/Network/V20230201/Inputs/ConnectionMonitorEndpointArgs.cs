// *** WARNING: this file was generated by pulumi. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.AzureNative.Network.V20230201.Inputs
{

    /// <summary>
    /// Describes the connection monitor endpoint.
    /// </summary>
    public sealed class ConnectionMonitorEndpointArgs : global::Pulumi.ResourceArgs
    {
        /// <summary>
        /// Address of the connection monitor endpoint (IP or domain name).
        /// </summary>
        [Input("address")]
        public Input<string>? Address { get; set; }

        /// <summary>
        /// Test coverage for the endpoint.
        /// </summary>
        [Input("coverageLevel")]
        public InputUnion<string, Pulumi.AzureNative.Network.V20230201.CoverageLevel>? CoverageLevel { get; set; }

        /// <summary>
        /// Filter for sub-items within the endpoint.
        /// </summary>
        [Input("filter")]
        public Input<Inputs.ConnectionMonitorEndpointFilterArgs>? Filter { get; set; }

        /// <summary>
        /// The name of the connection monitor endpoint.
        /// </summary>
        [Input("name", required: true)]
        public Input<string> Name { get; set; } = null!;

        /// <summary>
        /// Resource ID of the connection monitor endpoint.
        /// </summary>
        [Input("resourceId")]
        public Input<string>? ResourceId { get; set; }

        /// <summary>
        /// Endpoint scope.
        /// </summary>
        [Input("scope")]
        public Input<Inputs.ConnectionMonitorEndpointScopeArgs>? Scope { get; set; }

        /// <summary>
        /// The endpoint type.
        /// </summary>
        [Input("type")]
        public InputUnion<string, Pulumi.AzureNative.Network.V20230201.EndpointType>? Type { get; set; }

        public ConnectionMonitorEndpointArgs()
        {
        }
        public static new ConnectionMonitorEndpointArgs Empty => new ConnectionMonitorEndpointArgs();
    }
}
