// *** WARNING: this file was generated by pulumi. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.AzureNative.Redis.V20150801.Inputs
{

    /// <summary>
    /// SKU parameters supplied to the create Redis operation.
    /// </summary>
    public sealed class SkuArgs : global::Pulumi.ResourceArgs
    {
        /// <summary>
        /// What size of Redis cache to deploy. Valid values: for C family (0, 1, 2, 3, 4, 5, 6), for P family (1, 2, 3, 4).
        /// </summary>
        [Input("capacity", required: true)]
        public Input<int> Capacity { get; set; } = null!;

        /// <summary>
        /// Which family to use. Valid values: (C, P).
        /// </summary>
        [Input("family", required: true)]
        public InputUnion<string, Pulumi.AzureNative.Redis.V20150801.SkuFamily> Family { get; set; } = null!;

        /// <summary>
        /// What type of Redis cache to deploy. Valid values: (Basic, Standard, Premium).
        /// </summary>
        [Input("name", required: true)]
        public InputUnion<string, Pulumi.AzureNative.Redis.V20150801.SkuName> Name { get; set; } = null!;

        public SkuArgs()
        {
        }
        public static new SkuArgs Empty => new SkuArgs();
    }
}
