// *** WARNING: this file was generated by the Pulumi SDK Generator. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.AzureRM.ContainerService.Outputs
{

    [OutputType]
    public sealed class OpenShiftManagedClusterIdentityProviderResponse
    {
        /// <summary>
        /// Name of the provider.
        /// </summary>
        public readonly string? Name;
        /// <summary>
        /// Configuration of the provider.
        /// </summary>
        public readonly Outputs.OpenShiftManagedClusterBaseIdentityProviderResponse? Provider;

        [OutputConstructor]
        private OpenShiftManagedClusterIdentityProviderResponse(
            string? name,

            Outputs.OpenShiftManagedClusterBaseIdentityProviderResponse? provider)
        {
            Name = name;
            Provider = provider;
        }
    }
}
