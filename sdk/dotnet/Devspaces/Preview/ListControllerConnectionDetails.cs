// *** WARNING: this file was generated by the Pulumi SDK Generator. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.AzureRM.DevSpaces.Preview
{
    public static class ListControllerConnectionDetails
    {
        public static Task<ListControllerConnectionDetailsResult> InvokeAsync(ListControllerConnectionDetailsArgs args, InvokeOptions? options = null)
            => Pulumi.Deployment.Instance.InvokeAsync<ListControllerConnectionDetailsResult>("azurerm:devspaces/preview:listControllerConnectionDetails", args ?? new ListControllerConnectionDetailsArgs(), options.WithVersion());
    }


    public sealed class ListControllerConnectionDetailsArgs : Pulumi.InvokeArgs
    {
        /// <summary>
        /// Name of the resource.
        /// </summary>
        [Input("name", required: true)]
        public string Name { get; set; } = null!;

        /// <summary>
        /// Resource group to which the resource belongs.
        /// </summary>
        [Input("resourceGroupName", required: true)]
        public string ResourceGroupName { get; set; } = null!;

        /// <summary>
        /// Resource ID of the target container host mapped to the Azure Dev Spaces Controller.
        /// </summary>
        [Input("targetContainerHostResourceId", required: true)]
        public string TargetContainerHostResourceId { get; set; } = null!;

        public ListControllerConnectionDetailsArgs()
        {
        }
    }


    [OutputType]
    public sealed class ListControllerConnectionDetailsResult
    {
        /// <summary>
        /// List of Azure Dev Spaces Controller connection details.
        /// </summary>
        public readonly ImmutableArray<Outputs.ControllerConnectionDetailsResponseResult> ConnectionDetailsList;

        [OutputConstructor]
        private ListControllerConnectionDetailsResult(ImmutableArray<Outputs.ControllerConnectionDetailsResponseResult> connectionDetailsList)
        {
            ConnectionDetailsList = connectionDetailsList;
        }
    }
}
