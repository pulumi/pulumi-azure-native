// *** WARNING: this file was generated by the Pulumi SDK Generator. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.AzureRM.BatchAI.Inputs
{

    /// <summary>
    /// File Server mounting configuration.
    /// </summary>
    public sealed class FileServerReferenceResponseArgs : Pulumi.ResourceArgs
    {
        /// <summary>
        /// Resource ID of the existing File Server to be mounted.
        /// </summary>
        [Input("fileServer", required: true)]
        public Input<Inputs.ResourceIdResponseArgs> FileServer { get; set; } = null!;

        /// <summary>
        /// Mount options to be passed to mount command.
        /// </summary>
        [Input("mountOptions")]
        public Input<string>? MountOptions { get; set; }

        /// <summary>
        /// The relative path on the compute node where the File Server will be mounted. Note that all cluster level file servers will be mounted under $AZ_BATCHAI_MOUNT_ROOT location and all job level file servers will be mounted under $AZ_BATCHAI_JOB_MOUNT_ROOT.
        /// </summary>
        [Input("relativeMountPath", required: true)]
        public Input<string> RelativeMountPath { get; set; } = null!;

        /// <summary>
        /// File Server directory that needs to be mounted. If this property is not specified, the entire File Server will be mounted.
        /// </summary>
        [Input("sourceDirectory")]
        public Input<string>? SourceDirectory { get; set; }

        public FileServerReferenceResponseArgs()
        {
        }
    }
}
