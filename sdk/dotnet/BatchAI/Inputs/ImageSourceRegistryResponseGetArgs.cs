// *** WARNING: this file was generated by the Pulumi SDK Generator. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.AzureRM.BatchAI.Inputs
{

    /// <summary>
    /// Information about docker image for the job.
    /// </summary>
    public sealed class ImageSourceRegistryResponseGetArgs : Pulumi.ResourceArgs
    {
        /// <summary>
        /// Credentials to access the private docker repository.
        /// </summary>
        [Input("credentials")]
        public Input<Inputs.PrivateRegistryCredentialsResponseGetArgs>? Credentials { get; set; }

        /// <summary>
        /// The name of the image in the image repository.
        /// </summary>
        [Input("image", required: true)]
        public Input<string> Image { get; set; } = null!;

        /// <summary>
        /// URL for image repository.
        /// </summary>
        [Input("serverUrl")]
        public Input<string>? ServerUrl { get; set; }

        public ImageSourceRegistryResponseGetArgs()
        {
        }
    }
}
