// *** WARNING: this file was generated by the Pulumi SDK Generator. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.AzureNative.Elastic.V20230201Preview
{
    public static class GetOrganizationApiKey
    {
        /// <summary>
        /// The User Api Key created for the Organization associated with the User Email Id that was passed in the request
        /// </summary>
        public static Task<GetOrganizationApiKeyResult> InvokeAsync(GetOrganizationApiKeyArgs args, InvokeOptions? options = null)
            => global::Pulumi.Deployment.Instance.InvokeAsync<GetOrganizationApiKeyResult>("azure-native:elastic/v20230201preview:getOrganizationApiKey", args ?? new GetOrganizationApiKeyArgs(), options.WithDefaults());

        /// <summary>
        /// The User Api Key created for the Organization associated with the User Email Id that was passed in the request
        /// </summary>
        public static Output<GetOrganizationApiKeyResult> Invoke(GetOrganizationApiKeyInvokeArgs args, InvokeOptions? options = null)
            => global::Pulumi.Deployment.Instance.Invoke<GetOrganizationApiKeyResult>("azure-native:elastic/v20230201preview:getOrganizationApiKey", args ?? new GetOrganizationApiKeyInvokeArgs(), options.WithDefaults());
    }


    public sealed class GetOrganizationApiKeyArgs : global::Pulumi.InvokeArgs
    {
        /// <summary>
        /// The User email Id
        /// </summary>
        [Input("emailId")]
        public string? EmailId { get; set; }

        /// <summary>
        /// The name of the resource group to which the Elastic resource belongs.
        /// </summary>
        [Input("resourceGroupName", required: true)]
        public string ResourceGroupName { get; set; } = null!;

        public GetOrganizationApiKeyArgs()
        {
        }
        public static new GetOrganizationApiKeyArgs Empty => new GetOrganizationApiKeyArgs();
    }

    public sealed class GetOrganizationApiKeyInvokeArgs : global::Pulumi.InvokeArgs
    {
        /// <summary>
        /// The User email Id
        /// </summary>
        [Input("emailId")]
        public Input<string>? EmailId { get; set; }

        /// <summary>
        /// The name of the resource group to which the Elastic resource belongs.
        /// </summary>
        [Input("resourceGroupName", required: true)]
        public Input<string> ResourceGroupName { get; set; } = null!;

        public GetOrganizationApiKeyInvokeArgs()
        {
        }
        public static new GetOrganizationApiKeyInvokeArgs Empty => new GetOrganizationApiKeyInvokeArgs();
    }


    [OutputType]
    public sealed class GetOrganizationApiKeyResult
    {
        /// <summary>
        /// The User Api Key Generated based on ReturnApiKey flag. This is applicable for non-Portal clients only.
        /// </summary>
        public readonly string? ApiKey;

        [OutputConstructor]
        private GetOrganizationApiKeyResult(string? apiKey)
        {
            ApiKey = apiKey;
        }
    }
}
