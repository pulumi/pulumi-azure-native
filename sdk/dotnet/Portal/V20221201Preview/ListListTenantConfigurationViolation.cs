// *** WARNING: this file was generated by pulumi. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.AzureNative.Portal.V20221201Preview
{
    public static class ListListTenantConfigurationViolation
    {
        /// <summary>
        /// Gets list of items that violate tenant's configuration.
        /// </summary>
        public static Task<ListListTenantConfigurationViolationResult> InvokeAsync(ListListTenantConfigurationViolationArgs? args = null, InvokeOptions? options = null)
            => global::Pulumi.Deployment.Instance.InvokeAsync<ListListTenantConfigurationViolationResult>("azure-native:portal/v20221201preview:listListTenantConfigurationViolation", args ?? new ListListTenantConfigurationViolationArgs(), options.WithDefaults());

        /// <summary>
        /// Gets list of items that violate tenant's configuration.
        /// </summary>
        public static Output<ListListTenantConfigurationViolationResult> Invoke(InvokeOptions? options = null)
            => global::Pulumi.Deployment.Instance.Invoke<ListListTenantConfigurationViolationResult>("azure-native:portal/v20221201preview:listListTenantConfigurationViolation", InvokeArgs.Empty, options.WithDefaults());

        /// <summary>
        /// Gets list of items that violate tenant's configuration.
        /// </summary>
        public static Output<ListListTenantConfigurationViolationResult> Invoke(InvokeOutputOptions options)
            => global::Pulumi.Deployment.Instance.Invoke<ListListTenantConfigurationViolationResult>("azure-native:portal/v20221201preview:listListTenantConfigurationViolation", InvokeArgs.Empty, options.WithDefaults());
    }


    public sealed class ListListTenantConfigurationViolationArgs : global::Pulumi.InvokeArgs
    {
        public ListListTenantConfigurationViolationArgs()
        {
        }
        public static new ListListTenantConfigurationViolationArgs Empty => new ListListTenantConfigurationViolationArgs();
    }


    [OutputType]
    public sealed class ListListTenantConfigurationViolationResult
    {
        /// <summary>
        /// The link to the next page of items
        /// </summary>
        public readonly string? NextLink;
        /// <summary>
        /// The Violation items on this page
        /// </summary>
        public readonly ImmutableArray<Outputs.ViolationResponse> Value;

        [OutputConstructor]
        private ListListTenantConfigurationViolationResult(
            string? nextLink,

            ImmutableArray<Outputs.ViolationResponse> value)
        {
            NextLink = nextLink;
            Value = value;
        }
    }
}
