// *** WARNING: this file was generated by the Pulumi SDK Generator. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.AzureRM.DelegatedNetwork.Preview
{
    public static class GetControllerDetails
    {
        public static Task<GetControllerDetailsResult> InvokeAsync(GetControllerDetailsArgs args, InvokeOptions? options = null)
            => Pulumi.Deployment.Instance.InvokeAsync<GetControllerDetailsResult>("azurerm:delegatednetwork/preview:getControllerDetails", args ?? new GetControllerDetailsArgs(), options.WithVersion());
    }


    public sealed class GetControllerDetailsArgs : Pulumi.InvokeArgs
    {
        /// <summary>
        /// The name of the Azure Resource group of which a given DelegatedNetwork resource is part. This name must be at least 1 character in length, and no more than 90.
        /// </summary>
        [Input("resourceGroupName", required: true)]
        public string ResourceGroupName { get; set; } = null!;

        /// <summary>
        /// The name of the resource. It must be a minimum of 3 characters, and a maximum of 63.
        /// </summary>
        [Input("resourceName", required: true)]
        public string ResourceName { get; set; } = null!;

        public GetControllerDetailsArgs()
        {
        }
    }


    [OutputType]
    public sealed class GetControllerDetailsResult
    {
        /// <summary>
        /// Get controller AAD ID.
        /// </summary>
        public readonly string? DncAppID;
        /// <summary>
        /// Dnc Endpoint url.
        /// </summary>
        public readonly string? DncEndpoint;
        /// <summary>
        /// Location of the DNC controller resource.
        /// </summary>
        public readonly string? Location;
        /// <summary>
        /// The name of the DNC controller resource.
        /// </summary>
        public readonly string Name;
        /// <summary>
        /// Gets or sets resource GUID property of the controller resource.
        /// </summary>
        public readonly string? ResourceGuid;
        /// <summary>
        /// The current state of dnc controller resource.
        /// </summary>
        public readonly string State;
        /// <summary>
        /// The type of the DNC controller  resource.(Microsoft.DelegatedNetwork/controller)
        /// </summary>
        public readonly string Type;

        [OutputConstructor]
        private GetControllerDetailsResult(
            string? dncAppID,

            string? dncEndpoint,

            string? location,

            string name,

            string? resourceGuid,

            string state,

            string type)
        {
            DncAppID = dncAppID;
            DncEndpoint = dncEndpoint;
            Location = location;
            Name = name;
            ResourceGuid = resourceGuid;
            State = state;
            Type = type;
        }
    }
}
