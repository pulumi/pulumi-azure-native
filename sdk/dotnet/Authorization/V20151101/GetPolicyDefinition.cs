// *** WARNING: this file was generated by the Pulumi SDK Generator. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.AzureRM.Authorization.V20151101
{
    public static class GetPolicyDefinition
    {
        public static Task<GetPolicyDefinitionResult> InvokeAsync(GetPolicyDefinitionArgs args, InvokeOptions? options = null)
            => Pulumi.Deployment.Instance.InvokeAsync<GetPolicyDefinitionResult>("azurerm:authorization/v20151101:getPolicyDefinition", args ?? new GetPolicyDefinitionArgs(), options.WithVersion());
    }


    public sealed class GetPolicyDefinitionArgs : Pulumi.InvokeArgs
    {
        /// <summary>
        /// The policy definition name.
        /// </summary>
        [Input("policyDefinitionName", required: true)]
        public string PolicyDefinitionName { get; set; } = null!;

        public GetPolicyDefinitionArgs()
        {
        }
    }


    [OutputType]
    public sealed class GetPolicyDefinitionResult
    {
        /// <summary>
        /// Gets or sets the policy definition name.
        /// </summary>
        public readonly string? Name;
        /// <summary>
        /// Gets or sets the policy definition properties.
        /// </summary>
        public readonly Outputs.PolicyDefinitionPropertiesResponseResult Properties;

        [OutputConstructor]
        private GetPolicyDefinitionResult(
            string? name,

            Outputs.PolicyDefinitionPropertiesResponseResult properties)
        {
            Name = name;
            Properties = properties;
        }
    }
}
