// *** WARNING: this file was generated by the Pulumi SDK Generator. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.AzureRM.Billing.Latest
{
    public static class GetCostAllocationRule
    {
        public static Task<GetCostAllocationRuleResult> InvokeAsync(GetCostAllocationRuleArgs args, InvokeOptions? options = null)
            => Pulumi.Deployment.Instance.InvokeAsync<GetCostAllocationRuleResult>("azurerm:billing/latest:getCostAllocationRule", args ?? new GetCostAllocationRuleArgs(), options.WithVersion());
    }


    public sealed class GetCostAllocationRuleArgs : Pulumi.InvokeArgs
    {
        /// <summary>
        /// BillingAccount ID
        /// </summary>
        [Input("billingAccountId", required: true)]
        public string BillingAccountId { get; set; } = null!;

        /// <summary>
        /// Cost allocation rule name. The name cannot include spaces or any non alphanumeric characters other than '_' and '-'. The max length is 260 characters.
        /// </summary>
        [Input("ruleName", required: true)]
        public string RuleName { get; set; } = null!;

        public GetCostAllocationRuleArgs()
        {
        }
    }


    [OutputType]
    public sealed class GetCostAllocationRuleResult
    {
        /// <summary>
        /// Name of the rule. This is a read only value.
        /// </summary>
        public readonly string Name;
        /// <summary>
        /// Cost allocation rule properties
        /// </summary>
        public readonly Outputs.CostAllocationRulePropertiesResponseResult Properties;
        /// <summary>
        /// Resource type of the rule. This is a read only value of Microsoft.CostManagement/CostAllocationRule.
        /// </summary>
        public readonly string Type;

        [OutputConstructor]
        private GetCostAllocationRuleResult(
            string name,

            Outputs.CostAllocationRulePropertiesResponseResult properties,

            string type)
        {
            Name = name;
            Properties = properties;
            Type = type;
        }
    }
}
