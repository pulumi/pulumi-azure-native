// *** WARNING: this file was generated by the Pulumi SDK Generator. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.AzureRM.Devices.V20181201Preview.Outputs
{

    [OutputType]
    public sealed class IotHubPropertiesResponseDeviceStreamsResult
    {
        /// <summary>
        /// List of Device Streams Endpoints.
        /// </summary>
        public readonly ImmutableArray<string> StreamingEndpoints;

        [OutputConstructor]
        private IotHubPropertiesResponseDeviceStreamsResult(ImmutableArray<string> streamingEndpoints)
        {
            StreamingEndpoints = streamingEndpoints;
        }
    }
}
