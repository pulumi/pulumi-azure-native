// *** WARNING: this file was generated by the Pulumi SDK Generator. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;

namespace Pulumi.AzureRM.AzureStack.Preview
{
    public static class GetCustomerSubscription
    {
        public static Task<GetCustomerSubscriptionResult> InvokeAsync(GetCustomerSubscriptionArgs args, InvokeOptions? options = null)
            => Pulumi.Deployment.Instance.InvokeAsync<GetCustomerSubscriptionResult>("azurerm:azurestack/preview:getCustomerSubscription", args ?? new GetCustomerSubscriptionArgs(), options.WithVersion());
    }


    public sealed class GetCustomerSubscriptionArgs : Pulumi.InvokeArgs
    {
        /// <summary>
        /// Name of the product.
        /// </summary>
        [Input("customerSubscriptionName", required: true)]
        public string CustomerSubscriptionName { get; set; } = null!;

        /// <summary>
        /// Name of the Azure Stack registration.
        /// </summary>
        [Input("registrationName", required: true)]
        public string RegistrationName { get; set; } = null!;

        /// <summary>
        /// Name of the resource group.
        /// </summary>
        [Input("resourceGroup", required: true)]
        public string ResourceGroup { get; set; } = null!;

        public GetCustomerSubscriptionArgs()
        {
        }
    }


    [OutputType]
    public sealed class GetCustomerSubscriptionResult
    {
        /// <summary>
        /// The entity tag used for optimistic concurrency when modifying the resource.
        /// </summary>
        public readonly string? Etag;
        /// <summary>
        /// Name of the resource.
        /// </summary>
        public readonly string Name;
        /// <summary>
        /// Tenant Id.
        /// </summary>
        public readonly string? TenantId;
        /// <summary>
        /// Type of Resource.
        /// </summary>
        public readonly string Type;

        [OutputConstructor]
        private GetCustomerSubscriptionResult(
            string? etag,

            string name,

            string? tenantId,

            string type)
        {
            Etag = etag;
            Name = name;
            TenantId = tenantId;
            Type = type;
        }
    }
}
