# coding=utf-8
# *** WARNING: this file was generated by the Pulumi SDK Generator. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import warnings
import pulumi
import pulumi.runtime
from typing import Union
from .. import _utilities, _tables


class ListWorkspaceCollectionKeysResult:
    def __init__(__self__, key1=None, key2=None):
        if key1 and not isinstance(key1, str):
            raise TypeError("Expected argument 'key1' to be a str")
        __self__.key1 = key1
        """
        Access key 1
        """
        if key2 and not isinstance(key2, str):
            raise TypeError("Expected argument 'key2' to be a str")
        __self__.key2 = key2
        """
        Access key 2
        """


class AwaitableListWorkspaceCollectionKeysResult(ListWorkspaceCollectionKeysResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return ListWorkspaceCollectionKeysResult(
            key1=self.key1,
            key2=self.key2)


def list_workspace_collection_keys(resource_group_name=None, workspace_collection_name=None, opts=None):
    """
    Use this data source to access information about an existing resource.

    :param str resource_group_name: Azure resource group
    :param str workspace_collection_name: Power BI Embedded Workspace Collection name
    """
    __args__ = dict()
    __args__['resourceGroupName'] = resource_group_name
    __args__['workspaceCollectionName'] = workspace_collection_name
    if opts is None:
        opts = pulumi.InvokeOptions()
    if opts.version is None:
        opts.version = _utilities.get_version()
    __ret__ = pulumi.runtime.invoke('azurerm:powerbi:listWorkspaceCollectionKeys', __args__, opts=opts).value

    return AwaitableListWorkspaceCollectionKeysResult(
        key1=__ret__.get('key1'),
        key2=__ret__.get('key2'))
