# coding=utf-8
# *** WARNING: this file was generated by the Pulumi SDK Generator. ***
# *** Do not edit by hand unless you're certain you know what you are doing! ***

import warnings
import pulumi
import pulumi.runtime
from typing import Union
from .. import _utilities, _tables


class ListProvisioningServiceKeyKeysResult:
    """
    Description of the shared access key.
    """
    def __init__(__self__, key_name=None, primary_key=None, rights=None, secondary_key=None):
        if key_name and not isinstance(key_name, str):
            raise TypeError("Expected argument 'key_name' to be a str")
        __self__.key_name = key_name
        """
        Name of the key.
        """
        if primary_key and not isinstance(primary_key, str):
            raise TypeError("Expected argument 'primary_key' to be a str")
        __self__.primary_key = primary_key
        """
        Primary SAS key value.
        """
        if rights and not isinstance(rights, str):
            raise TypeError("Expected argument 'rights' to be a str")
        __self__.rights = rights
        """
        Rights that this key has.
        """
        if secondary_key and not isinstance(secondary_key, str):
            raise TypeError("Expected argument 'secondary_key' to be a str")
        __self__.secondary_key = secondary_key
        """
        Secondary SAS key value.
        """


class AwaitableListProvisioningServiceKeyKeysResult(ListProvisioningServiceKeyKeysResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return ListProvisioningServiceKeyKeysResult(
            key_name=self.key_name,
            primary_key=self.primary_key,
            rights=self.rights,
            secondary_key=self.secondary_key)


def list_provisioning_service_key_keys(key_name=None, provisioning_service_name=None, resource_group_name=None, opts=None):
    """
    Use this data source to access information about an existing resource.

    :param str key_name: Logical key name to get key-values for.
    :param str provisioning_service_name: Name of the provisioning service.
    :param str resource_group_name: The name of the resource group that contains the provisioning service.
    """
    __args__ = dict()
    __args__['keyName'] = key_name
    __args__['provisioningServiceName'] = provisioning_service_name
    __args__['resourceGroupName'] = resource_group_name
    if opts is None:
        opts = pulumi.InvokeOptions()
    if opts.version is None:
        opts.version = _utilities.get_version()
    __ret__ = pulumi.runtime.invoke('azurerm:devices:listProvisioningServiceKeyKeys', __args__, opts=opts).value

    return AwaitableListProvisioningServiceKeyKeysResult(
        key_name=__ret__.get('keyName'),
        primary_key=__ret__.get('primaryKey'),
        rights=__ret__.get('rights'),
        secondary_key=__ret__.get('secondaryKey'))
