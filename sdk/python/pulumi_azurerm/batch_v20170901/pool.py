...
class Pool(pulumi.CustomResource):
    ...
    The properties associated with the pool.
      * `allocation_state` (`str`) - Values are:
      ...
      * `application_packages` (`list`) - Changes to application packages affect all new compute nodes joining the pool, but do not affect compute nodes that are already in the pool until they are rebooted or reimaged.
        * `id` (`str`)
      ...
      * `deployment_configuration` (`dict`) - Using CloudServiceConfiguration specifies that the nodes should be creating using Azure Cloud Services (PaaS), while VirtualMachineConfiguration uses Azure Virtual Machines (IaaS).
        * `cloud_service_configuration` (`dict`) - This property and virtualMachineConfiguration are mutually exclusive and one of the properties must be specified. This property cannot be specified if the Batch account was created with its poolAllocationMode property set to 'UserSubscription'.
          * `current_os_version` (`str`) - This may differ from targetOSVersion if the pool state is Upgrading. In this case some virtual machines may be on the targetOSVersion and some may be on the currentOSVersion during the upgrade process. Once all virtual machines have upgraded, currentOSVersion is updated to be the same as targetOSVersion.
          ...
          * `target_os_version` (`str`) - The default value is * which specifies the latest operating system version for the specified OS family.
        ...
        * `virtual_machine_configuration` (`dict`) - This property and cloudServiceConfiguration are mutually exclusive and one of the properties must be specified.
          ...
          * `os_disk` (`dict`)
            * `caching` (`str`) - Default value is none.
      ...
      * `provisioning_state` (`str`) - Values are:
      ...
      * `start_task` (`dict`) - In an PATCH (update) operation, this property can be set to an empty object to remove the start task from the pool.
        ...
        * `user_identity` (`dict`) - If omitted, the task runs as a non-administrative user unique to the task.
          * `auto_user` (`dict`) - The userName and autoUser properties are mutually exclusive; you must specify one but not both.
            ...
            * `scope` (`str`) - pool - specifies that the task runs as the common auto user account which is created on every node in a pool. task - specifies that the service should create a new user for the task. The default value is task.
    ...
    def __init__(__self__, resource_name, opts=None, account_name=None, name=None, properties=None, resource_group_name=None, __props__=None, __name__=None, __opts__=None):
        ...
        The **properties** object supports the following:
          ...
          * `application_packages` (`pulumi.Input[list]`) - Changes to application packages affect all new compute nodes joining the pool, but do not affect compute nodes that are already in the pool until they are rebooted or reimaged.
            * `id` (`pulumi.Input[str]`)
          ...
          * `certificates` (`pulumi.Input[list]`) - For Windows compute nodes, the Batch service installs the certificates to the specified certificate store and location. For Linux compute nodes, the certificates are stored in a directory inside the task working directory and an environment variable AZ_BATCH_CERTIFICATES_DIR is supplied to the task to query for this location. For certificates with visibility of 'remoteUser', a 'certs' directory is created in the user's home directory (e.g., /home/{user-name}/certs) and certificates are placed in that directory.
            ...
            * `visibility` (`pulumi.Input[list]`) - Values are:
          ...
          * `deployment_configuration` (`pulumi.Input[dict]`) - Using CloudServiceConfiguration specifies that the nodes should be creating using Azure Cloud Services (PaaS), while VirtualMachineConfiguration uses Azure Virtual Machines (IaaS).
            * `cloud_service_configuration` (`pulumi.Input[dict]`) - This property and virtualMachineConfiguration are mutually exclusive and one of the properties must be specified. This property cannot be specified if the Batch account was created with its poolAllocationMode property set to 'UserSubscription'.
              * `current_os_version` (`pulumi.Input[str]`) - This may differ from targetOSVersion if the pool state is Upgrading. In this case some virtual machines may be on the targetOSVersion and some may be on the currentOSVersion during the upgrade process. Once all virtual machines have upgraded, currentOSVersion is updated to be the same as targetOSVersion.
              * `os_family` (`pulumi.Input[str]`) - Possible values are: 2 - OS Family 2, equivalent to Windows Server 2008 R2 SP1. 3 - OS Family 3, equivalent to Windows Server 2012. 4 - OS Family 4, equivalent to Windows Server 2012 R2. 5 - OS Family 5, equivalent to Windows Server 2016. For more information, see Azure Guest OS Releases (https://azure.microsoft.com/documentation/articles/cloud-services-guestos-update-matrix/#releases).
              * `target_os_version` (`pulumi.Input[str]`) - The default value is * which specifies the latest operating system version for the specified OS family.
            ...
            * `virtual_machine_configuration` (`pulumi.Input[dict]`) - This property and cloudServiceConfiguration are mutually exclusive and one of the properties must be specified.
              ...
              * `os_disk` (`pulumi.Input[dict]`)
                * `caching` (`pulumi.Input[str]`) - Default value is none.
          ...
          * `max_tasks_per_node` (`pulumi.Input[float]`)
          ...
          * `start_task` (`pulumi.Input[dict]`) - In an PATCH (update) operation, this property can be set to an empty object to remove the start task from the pool.
            ...
            * `resource_files` (`pulumi.Input[list]`)
              * `blob_source` (`pulumi.Input[str]`) - This URL must be readable using anonymous access; that is, the Batch service does not present any credentials when downloading the blob. There are two ways to get such a URL for a blob in Azure storage: include a Shared Access Signature (SAS) granting read permissions on the blob, or set the ACL for the blob or its container to allow public access.
            ...
            * `user_identity` (`pulumi.Input[dict]`) - If omitted, the task runs as a non-administrative user unique to the task.
              * `auto_user` (`pulumi.Input[dict]`) - The userName and autoUser properties are mutually exclusive; you must specify one but not both.
                ...
                * `scope` (`pulumi.Input[str]`) - pool - specifies that the task runs as the common auto user account which is created on every node in a pool. task - specifies that the service should create a new user for the task. The default value is task.
          ...
          * `task_scheduling_policy` (`pulumi.Input[dict]`)
