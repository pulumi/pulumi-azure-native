...
class Pool(pulumi.CustomResource):
    ...
    def __init__(__self__, resource_name, opts=None, account_name=None, name=None, properties=None, resource_group_name=None, __props__=None, __name__=None, __opts__=None):
        ...
        The **properties** object supports the following:
          ...
          * `deployment_configuration` (`pulumi.Input[dict]`) - Using CloudServiceConfiguration specifies that the nodes should be creating using Azure Cloud Services (PaaS), while VirtualMachineConfiguration uses Azure Virtual Machines (IaaS).
            ...
            * `virtual_machine_configuration` (`pulumi.Input[dict]`) - This property and cloudServiceConfiguration are mutually exclusive and one of the properties must be specified.
              ...
              * `image_reference` (`pulumi.Input[dict]`)
                * `id` (`pulumi.Input[str]`) - This property is mutually exclusive with other properties. For Virtual Machine Image it must be in the same region and subscription as the Azure Batch account. For SIG image it must have replicas in the same region as the Azure Batch account. For information about the firewall settings for the Batch node agent to communicate with the Batch service see https://docs.microsoft.com/en-us/azure/batch/batch-api-basics#virtual-network-vnet-and-firewall-configuration.
                ...
                * `sku` (`pulumi.Input[str]`) - For example, 18.04-LTS or 2019-Datacenter.
          ...
          * `mount_configuration` (`pulumi.Input[list]`) - This supports Azure Files, NFS, CIFS/SMB, and Blobfuse.
            * `azure_blob_file_system_configuration` (`pulumi.Input[dict]`) - This property is mutually exclusive with all other properties.
              * `account_key` (`pulumi.Input[str]`) - This property is mutually exclusive with sasKey and one must be specified.
              * `account_name` (`pulumi.Input[str]`)
              * `blobfuse_options` (`pulumi.Input[str]`) - These are 'net use' options in Windows and 'mount' options in Linux.
              * `container_name` (`pulumi.Input[str]`)
              * `relative_mount_path` (`pulumi.Input[str]`) - All file systems are mounted relative to the Batch mounts directory, accessible via the AZ_BATCH_NODE_MOUNTS_DIR environment variable.
              * `sas_key` (`pulumi.Input[str]`) - This property is mutually exclusive with accountKey and one must be specified.

            * `azure_file_share_configuration` (`pulumi.Input[dict]`) - This property is mutually exclusive with all other properties.
              * `account_key` (`pulumi.Input[str]`)
              * `account_name` (`pulumi.Input[str]`)
              * `azure_file_url` (`pulumi.Input[str]`) - This is of the form 'https://{account}.file.core.windows.net/'.
              * `mount_options` (`pulumi.Input[str]`) - These are 'net use' options in Windows and 'mount' options in Linux.
              * `relative_mount_path` (`pulumi.Input[str]`) - All file systems are mounted relative to the Batch mounts directory, accessible via the AZ_BATCH_NODE_MOUNTS_DIR environment variable.

            * `cifs_mount_configuration` (`pulumi.Input[dict]`) - This property is mutually exclusive with all other properties.
              * `mount_options` (`pulumi.Input[str]`) - These are 'net use' options in Windows and 'mount' options in Linux.
              * `password` (`pulumi.Input[str]`)
              * `relative_mount_path` (`pulumi.Input[str]`) - All file systems are mounted relative to the Batch mounts directory, accessible via the AZ_BATCH_NODE_MOUNTS_DIR environment variable.
            ...
            * `nfs_mount_configuration` (`pulumi.Input[dict]`) - This property is mutually exclusive with all other properties.
              * `mount_options` (`pulumi.Input[str]`) - These are 'net use' options in Windows and 'mount' options in Linux.
              * `relative_mount_path` (`pulumi.Input[str]`) - All file systems are mounted relative to the Batch mounts directory, accessible via the AZ_BATCH_NODE_MOUNTS_DIR environment variable.
              * `source` (`pulumi.Input[str]`)
            ...
            * `endpoint_configuration` (`pulumi.Input[dict]`) - Pool endpoint configuration is only supported on pools with the virtualMachineConfiguration property.
              * `inbound_nat_pools` (`pulumi.Input[list]`) - The maximum number of inbound NAT pools per Batch pool is 5. If the maximum number of inbound NAT pools is exceeded the request fails with HTTP status code 400.
                ...
                * `network_security_group_rules` (`pulumi.Input[list]`) - The maximum number of rules that can be specified across all the endpoints on a Batch pool is 25. If no network security group rules are specified, a default rule will be created to allow inbound access to the specified backendPort. If the maximum number of network security group rules is exceeded the request fails with HTTP status code 400.
                  ...
                  * `source_port_ranges` (`pulumi.Input[list]`) - Valid values are '*' (for all ports 0 - 65535) or arrays of ports or port ranges (i.e. 100-200). The ports should in the range of 0 to 65535 and the port ranges or ports can't overlap. If any other values are provided the request fails with HTTP status code 400. Default value will be *.

                * `protocol` (`pulumi.Input[str]`)

            * `public_i_ps` (`pulumi.Input[list]`) - The number of IPs specified here limits the maximum size of the Pool - 50 dedicated nodes or 20 low-priority nodes can be allocated for each public IP. For example, a pool needing 150 dedicated VMs would need at least 3 public IPs specified. Each element of this collection is of the form: /subscriptions/{subscription}/resourceGroups/{group}/providers/Microsoft.Network/publicIPAddresses/{ip}.
          ...
          * `start_task` (`pulumi.Input[dict]`) - In an PATCH (update) operation, this property can be set to an empty object to remove the start task from the pool.
            ...
            * `user_identity` (`pulumi.Input[dict]`) - If omitted, the task runs as a non-administrative user unique to the task.
              * `auto_user` (`pulumi.Input[dict]`) - The userName and autoUser properties are mutually exclusive; you must specify one but not both.
                * `elevation_level` (`pulumi.Input[str]`) - The default value is nonAdmin.
                * `scope` (`pulumi.Input[str]`) - The default value is Pool. If the pool is running Windows a value of Task should be specified if stricter isolation between tasks is required. For example, if the task mutates the registry in a way which could impact other tasks, or if certificates have been specified on the pool which should not be accessible by normal tasks but should be accessible by start tasks.
