...
class Server(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if not isinstance(backup, BackupArgs):
                backup = backup or {}
                def _setter(key, value):
                    backup[key] = value
            ...
            if not isinstance(data_encryption, DataEncryptionArgs):
            ...
            if not isinstance(high_availability, HighAvailabilityArgs):
            ...
            if not isinstance(maintenance_window, MaintenanceWindowArgs):
            ...
            if not isinstance(network, NetworkArgs):
                network = network or {}
                def _setter(key, value):
                    network[key] = value
            ...
            if not isinstance(storage, StorageArgs):
                storage = storage or {}
                def _setter(key, value):
                    storage[key] = value
