...
class VirtualmachineRetrieveArgs:
    ...
    def _configure(
        ...
        if 'virtualmachinesName' in kwargs:
...
class VirtualmachineRetrieve(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if hardware_profile is not None and not isinstance(hardware_profile, VirtualmachinesPropertiesHardwareProfileArgs):
                hardware_profile = hardware_profile or {}
                def _setter(key, value):
                    hardware_profile[key] = value
                VirtualmachinesPropertiesHardwareProfileArgs._configure(_setter, **hardware_profile)
            ...
            if network_profile is not None and not isinstance(network_profile, VirtualmachinesPropertiesNetworkProfileArgs):
                network_profile = network_profile or {}
                def _setter(key, value):
                    network_profile[key] = value
                VirtualmachinesPropertiesNetworkProfileArgs._configure(_setter, **network_profile)
            __props__.__dict__["network_profile"] = network_profile
            if os_profile is not None and not isinstance(os_profile, VirtualmachinesPropertiesOsProfileArgs):
                os_profile = os_profile or {}
                def _setter(key, value):
                    os_profile[key] = value
                VirtualmachinesPropertiesOsProfileArgs._configure(_setter, **os_profile)
            __props__.__dict__["os_profile"] = os_profile
            if resource_group_name is None and not opts.urn:
                raise TypeError("Missing required property 'resource_group_name'")
            __props__.__dict__["resource_group_name"] = resource_group_name
            __props__.__dict__["resource_name"] = resource_name_
            if security_profile is not None and not isinstance(security_profile, VirtualmachinesPropertiesSecurityProfileArgs):
                security_profile = security_profile or {}
                def _setter(key, value):
                    security_profile[key] = value
                VirtualmachinesPropertiesSecurityProfileArgs._configure(_setter, **security_profile)
            __props__.__dict__["security_profile"] = security_profile
            if storage_profile is not None and not isinstance(storage_profile, VirtualmachinesPropertiesStorageProfileArgs):
                storage_profile = storage_profile or {}
                def _setter(key, value):
                    storage_profile[key] = value
                VirtualmachinesPropertiesStorageProfileArgs._configure(_setter, **storage_profile)
