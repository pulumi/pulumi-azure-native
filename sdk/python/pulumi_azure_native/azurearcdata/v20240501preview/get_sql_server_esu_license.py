...
class AwaitableGetSqlServerEsuLicenseResult(GetSqlServerEsuLicenseResult):
    ...
    def __await__(self):
        if False:
            yield self
        return GetSqlServerEsuLicenseResult(
            id=self.id,
...
def get_sql_server_esu_license(resource_group_name: Optional[str] = None,
    ...
    __ret__ = pulumi.runtime.invoke('azure-native:azurearcdata/v20240501preview:getSqlServerEsuLicense', __args__, opts=opts, typ=GetSqlServerEsuLicenseResult).value

    return AwaitableGetSqlServerEsuLicenseResult(
...
@_utilities.lift_output_func(get_sql_server_esu_license)
