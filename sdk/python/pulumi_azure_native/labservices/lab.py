...
class LabArgs:
    ...
    def _configure(
             ...
             **kwargs):
        if 'autoShutdownProfile' in kwargs:
            auto_shutdown_profile = kwargs['autoShutdownProfile']
        if 'connectionProfile' in kwargs:
        ...
        if 'virtualMachineProfile' in kwargs:
...
class Lab(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if auto_shutdown_profile is not None and not isinstance(auto_shutdown_profile, AutoShutdownProfileArgs):
                auto_shutdown_profile = auto_shutdown_profile or {}
                def _setter(key, value):
                    auto_shutdown_profile[key] = value
                AutoShutdownProfileArgs._configure(_setter, **auto_shutdown_profile)
            ...
            if connection_profile is not None and not isinstance(connection_profile, ConnectionProfileArgs):
                connection_profile = connection_profile or {}
                def _setter(key, value):
                    connection_profile[key] = value
                ConnectionProfileArgs._configure(_setter, **connection_profile)
            ...
            if network_profile is not None and not isinstance(network_profile, LabNetworkProfileArgs):
                network_profile = network_profile or {}
                def _setter(key, value):
                    network_profile[key] = value
                LabNetworkProfileArgs._configure(_setter, **network_profile)
            __props__.__dict__["network_profile"] = network_profile
            if resource_group_name is None and not opts.urn:
                raise TypeError("Missing required property 'resource_group_name'")
            __props__.__dict__["resource_group_name"] = resource_group_name
            if roster_profile is not None and not isinstance(roster_profile, RosterProfileArgs):
                roster_profile = roster_profile or {}
                def _setter(key, value):
                    roster_profile[key] = value
                RosterProfileArgs._configure(_setter, **roster_profile)
            ...
            if virtual_machine_profile is not None and not isinstance(virtual_machine_profile, VirtualMachineProfileArgs):
                virtual_machine_profile = virtual_machine_profile or {}
                def _setter(key, value):
                    virtual_machine_profile[key] = value
                VirtualMachineProfileArgs._configure(_setter, **virtual_machine_profile)
