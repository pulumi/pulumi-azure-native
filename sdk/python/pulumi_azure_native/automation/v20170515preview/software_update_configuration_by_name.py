...
class SoftwareUpdateConfigurationByName(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if not isinstance(error, ErrorResponseArgs):
                error = error or {}
                def _setter(key, value):
                    error[key] = value
            ...
            if not isinstance(schedule_info, SchedulePropertiesArgs):
                schedule_info = schedule_info or {}
                def _setter(key, value):
                    schedule_info[key] = value
                SchedulePropertiesArgs._configure(_setter, **schedule_info)
            ...
            if not isinstance(tasks, SoftwareUpdateConfigurationTasksArgs):
                tasks = tasks or {}
                def _setter(key, value):
                    tasks[key] = value
                SoftwareUpdateConfigurationTasksArgs._configure(_setter, **tasks)
            __props__.__dict__["tasks"] = tasks
            if not isinstance(update_configuration, UpdateConfigurationArgs):
