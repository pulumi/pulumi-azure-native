...
class ServerEndpointArgs:
    ...
    def _configure(
        ...
        if cloud_tiering is None and 'cloudTiering' in kwargs:
            cloud_tiering = kwargs['cloudTiering']
        if friendly_name is None and 'friendlyName' in kwargs:
            friendly_name = kwargs['friendlyName']
        if initial_download_policy is None and 'initialDownloadPolicy' in kwargs:
            initial_download_policy = kwargs['initialDownloadPolicy']
        if initial_upload_policy is None and 'initialUploadPolicy' in kwargs:
            initial_upload_policy = kwargs['initialUploadPolicy']
        if local_cache_mode is None and 'localCacheMode' in kwargs:
            local_cache_mode = kwargs['localCacheMode']
        if offline_data_transfer is None and 'offlineDataTransfer' in kwargs:
            offline_data_transfer = kwargs['offlineDataTransfer']
        if offline_data_transfer_share_name is None and 'offlineDataTransferShareName' in kwargs:
            offline_data_transfer_share_name = kwargs['offlineDataTransferShareName']
        if server_endpoint_name is None and 'serverEndpointName' in kwargs:
            server_endpoint_name = kwargs['serverEndpointName']
        if server_local_path is None and 'serverLocalPath' in kwargs:
        ...
        if tier_files_older_than_days is None and 'tierFilesOlderThanDays' in kwargs:
            tier_files_older_than_days = kwargs['tierFilesOlderThanDays']
        if volume_free_space_percent is None and 'volumeFreeSpacePercent' in kwargs:
