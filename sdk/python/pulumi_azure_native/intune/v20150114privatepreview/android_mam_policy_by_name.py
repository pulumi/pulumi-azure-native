...
class AndroidMAMPolicyByNameArgs:
    ...
    def _configure(
        ...
        if access_recheck_offline_timeout is None and 'accessRecheckOfflineTimeout' in kwargs:
            access_recheck_offline_timeout = kwargs['accessRecheckOfflineTimeout']
        if access_recheck_online_timeout is None and 'accessRecheckOnlineTimeout' in kwargs:
            access_recheck_online_timeout = kwargs['accessRecheckOnlineTimeout']
        if app_sharing_from_level is None and 'appSharingFromLevel' in kwargs:
            app_sharing_from_level = kwargs['appSharingFromLevel']
        if app_sharing_to_level is None and 'appSharingToLevel' in kwargs:
            app_sharing_to_level = kwargs['appSharingToLevel']
        if clipboard_sharing_level is None and 'clipboardSharingLevel' in kwargs:
            clipboard_sharing_level = kwargs['clipboardSharingLevel']
        if data_backup is None and 'dataBackup' in kwargs:
            data_backup = kwargs['dataBackup']
        if device_compliance is None and 'deviceCompliance' in kwargs:
            device_compliance = kwargs['deviceCompliance']
        if file_encryption is None and 'fileEncryption' in kwargs:
            file_encryption = kwargs['fileEncryption']
        if file_sharing_save_as is None and 'fileSharingSaveAs' in kwargs:
            file_sharing_save_as = kwargs['fileSharingSaveAs']
        if managed_browser is None and 'managedBrowser' in kwargs:
            managed_browser = kwargs['managedBrowser']
        if offline_wipe_timeout is None and 'offlineWipeTimeout' in kwargs:
            offline_wipe_timeout = kwargs['offlineWipeTimeout']
        if pin_num_retry is None and 'pinNumRetry' in kwargs:
            pin_num_retry = kwargs['pinNumRetry']
        if policy_name is None and 'policyName' in kwargs:
            policy_name = kwargs['policyName']
        if screen_capture is None and 'screenCapture' in kwargs:
            screen_capture = kwargs['screenCapture']
