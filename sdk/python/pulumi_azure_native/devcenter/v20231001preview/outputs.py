...
class CatalogSyncErrorResponse(dict):
    ...
    def _configure(
             _setter: Callable[[Any, Any], None],
             error_details: Optional[Sequence['outputs.CatalogErrorDetailsResponse']] = None,
             path: Optional[str] = None,
             opts: Optional[pulumi.ResourceOptions] = None,
             **kwargs):
...
class SyncStatsResponse(dict):
    ...
    def _configure(
             ...
             synchronization_errors: Optional[int] = None,
             ...
             **kwargs):
        if added is None:
            raise TypeError("Missing 'added' argument")
        if removed is None:
            raise TypeError("Missing 'removed' argument")
        if synchronization_errors is None and 'synchronizationErrors' in kwargs:
            synchronization_errors = kwargs['synchronizationErrors']
        if synchronization_errors is None:
            raise TypeError("Missing 'synchronization_errors' argument")
        if unchanged is None:
            raise TypeError("Missing 'unchanged' argument")
        if updated is None:
