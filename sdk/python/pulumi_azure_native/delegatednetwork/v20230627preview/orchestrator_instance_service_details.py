...
class OrchestratorInstanceServiceDetailsArgs:
    ...
    def _configure(
             ...
             **kwargs):
        if controller_details is None and 'controllerDetails' in kwargs:
            controller_details = kwargs['controllerDetails']
        if controller_details is None:
            raise TypeError("Missing 'controller_details' argument")
        if kind is None:
        ...
        if api_server_endpoint is None and 'apiServerEndpoint' in kwargs:
            api_server_endpoint = kwargs['apiServerEndpoint']
        if cluster_root_ca is None and 'clusterRootCA' in kwargs:
            cluster_root_ca = kwargs['clusterRootCA']
        if orchestrator_app_id is None and 'orchestratorAppId' in kwargs:
            orchestrator_app_id = kwargs['orchestratorAppId']
        if orchestrator_tenant_id is None and 'orchestratorTenantId' in kwargs:
...
class OrchestratorInstanceServiceDetails(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            controller_details = _utilities.configure(controller_details, ControllerDetailsArgs, True)
            ...
            identity = _utilities.configure(identity, OrchestratorIdentityArgs, True)
        ...
        super(OrchestratorInstanceServiceDetails, __self__).__init__(
            'azure-native:delegatednetwork/v20230627preview:OrchestratorInstanceServiceDetails',
            resource_name,
            __props__,
            opts)
