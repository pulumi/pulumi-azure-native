...
class StorageAccountArgs:
    ...
    def _configure(
        ...
        if allow_blob_public_access is None and 'allowBlobPublicAccess' in kwargs:
            allow_blob_public_access = kwargs['allowBlobPublicAccess']
        if allow_cross_tenant_replication is None and 'allowCrossTenantReplication' in kwargs:
            allow_cross_tenant_replication = kwargs['allowCrossTenantReplication']
        if allow_shared_key_access is None and 'allowSharedKeyAccess' in kwargs:
            allow_shared_key_access = kwargs['allowSharedKeyAccess']
        if allowed_copy_scope is None and 'allowedCopyScope' in kwargs:
            allowed_copy_scope = kwargs['allowedCopyScope']
        if azure_files_identity_based_authentication is None and 'azureFilesIdentityBasedAuthentication' in kwargs:
            azure_files_identity_based_authentication = kwargs['azureFilesIdentityBasedAuthentication']
        if custom_domain is None and 'customDomain' in kwargs:
            custom_domain = kwargs['customDomain']
        if default_to_o_auth_authentication is None and 'defaultToOAuthAuthentication' in kwargs:
            default_to_o_auth_authentication = kwargs['defaultToOAuthAuthentication']
        if dns_endpoint_type is None and 'dnsEndpointType' in kwargs:
            dns_endpoint_type = kwargs['dnsEndpointType']
        ...
        if enable_nfs_v3 is None and 'enableNfsV3' in kwargs:
            enable_nfs_v3 = kwargs['enableNfsV3']
        if extended_location is None and 'extendedLocation' in kwargs:
            extended_location = kwargs['extendedLocation']
        if immutable_storage_with_versioning is None and 'immutableStorageWithVersioning' in kwargs:
            immutable_storage_with_versioning = kwargs['immutableStorageWithVersioning']
        if is_hns_enabled is None and 'isHnsEnabled' in kwargs:
            is_hns_enabled = kwargs['isHnsEnabled']
        if is_local_user_enabled is None and 'isLocalUserEnabled' in kwargs:
            is_local_user_enabled = kwargs['isLocalUserEnabled']
        if is_sftp_enabled is None and 'isSftpEnabled' in kwargs:
            is_sftp_enabled = kwargs['isSftpEnabled']
        if key_policy is None and 'keyPolicy' in kwargs:
            key_policy = kwargs['keyPolicy']
        if large_file_shares_state is None and 'largeFileSharesState' in kwargs:
            large_file_shares_state = kwargs['largeFileSharesState']
...
class StorageAccount(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            azure_files_identity_based_authentication = _utilities.configure(azure_files_identity_based_authentication, AzureFilesIdentityBasedAuthenticationArgs, True)
            __props__.__dict__["azure_files_identity_based_authentication"] = azure_files_identity_based_authentication
            custom_domain = _utilities.configure(custom_domain, CustomDomainArgs, True)
            ...
            immutable_storage_with_versioning = _utilities.configure(immutable_storage_with_versioning, ImmutableStorageAccountArgs, True)
            ...
            key_policy = _utilities.configure(key_policy, KeyPolicyArgs, True)
            ...
            routing_preference = _utilities.configure(routing_preference, RoutingPreferenceArgs, True)
            __props__.__dict__["routing_preference"] = routing_preference
            sas_policy = _utilities.configure(sas_policy, SasPolicyArgs, True)
