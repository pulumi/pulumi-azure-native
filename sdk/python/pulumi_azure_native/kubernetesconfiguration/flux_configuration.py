...
class FluxConfigurationArgs:
    ...
    def _configure(
        ...
        if flux_configuration_name is None and 'fluxConfigurationName' in kwargs:
        ...
        if reconciliation_wait_duration is None and 'reconciliationWaitDuration' in kwargs:
            reconciliation_wait_duration = kwargs['reconciliationWaitDuration']
        if source_kind is None and 'sourceKind' in kwargs:
            source_kind = kwargs['sourceKind']
        if wait_for_reconciliation is None and 'waitForReconciliation' in kwargs:
...
class FluxConfiguration(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            azure_blob = _utilities.configure(azure_blob, AzureBlobDefinitionArgs, True)
            __props__.__dict__["azure_blob"] = azure_blob
            bucket = _utilities.configure(bucket, BucketDefinitionArgs, True)
            ...
            git_repository = _utilities.configure(git_repository, GitRepositoryDefinitionArgs, True)
