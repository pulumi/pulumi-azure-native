...
class RouteFilterArgs:
    ...
    def _configure(
        ...
        if ipv6_peerings is None and 'ipv6Peerings' in kwargs:
            ipv6_peerings = kwargs['ipv6Peerings']
        if route_filter_name is None and 'routeFilterName' in kwargs:
            route_filter_name = kwargs['routeFilterName']

        _setter("resource_group_name", resource_group_name)
        if id is not None:
            _setter("id", id)
        if ipv6_peerings is not None:
            _setter("ipv6_peerings", ipv6_peerings)
        if location is not None:
            _setter("location", location)
        if peerings is not None:
            _setter("peerings", peerings)
        if route_filter_name is not None:
            _setter("route_filter_name", route_filter_name)
    ...
    def __init__(__self__, resource_name: str, *args, **kwargs):
        ...
        if resource_args is not None:
            ...
            def _setter(key, value):
                kwargs[key] = value
            RouteFilterArgs._configure(_setter, **kwargs)
