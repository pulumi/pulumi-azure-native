...
class BastionHostArgs:
    ...
    def _configure(
        ...
        if bastion_host_name is None and 'bastionHostName' in kwargs:
            bastion_host_name = kwargs['bastionHostName']
        if disable_copy_paste is None and 'disableCopyPaste' in kwargs:
            disable_copy_paste = kwargs['disableCopyPaste']
        if dns_name is None and 'dnsName' in kwargs:
            dns_name = kwargs['dnsName']
        if enable_file_copy is None and 'enableFileCopy' in kwargs:
            enable_file_copy = kwargs['enableFileCopy']
        if enable_ip_connect is None and 'enableIpConnect' in kwargs:
        ...
        if enable_shareable_link is None and 'enableShareableLink' in kwargs:
            enable_shareable_link = kwargs['enableShareableLink']
        if enable_tunneling is None and 'enableTunneling' in kwargs:
...
class BastionHost(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            network_acls = _utilities.configure(network_acls, BastionHostPropertiesFormatNetworkAclsArgs, True)
