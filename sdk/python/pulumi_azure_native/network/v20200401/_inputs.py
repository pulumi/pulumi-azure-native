...
class FirewallPolicyNatRuleArgs:
    ...
    def _configure(
        ...
        if rule_type is None:
            raise TypeError("Missing 'rule_type' argument")
        if rule_condition is None and 'ruleCondition' in kwargs:
...
class HubVirtualNetworkConnectionArgs:
    ...
    def _configure(
             ...
             opts: Optional[pulumi.ResourceOptions] = None,
             **kwargs):
        if allow_hub_to_remote_vnet_transit is None and 'allowHubToRemoteVnetTransit' in kwargs:
            allow_hub_to_remote_vnet_transit = kwargs['allowHubToRemoteVnetTransit']
        if allow_remote_vnet_to_use_hub_vnet_gateways is None and 'allowRemoteVnetToUseHubVnetGateways' in kwargs:
...
class NatRuleConditionArgs:
    ...
    def _configure(
        ...
        if destination_ports is None and 'destinationPorts' in kwargs:
            destination_ports = kwargs['destinationPorts']
        if ip_protocols is None and 'ipProtocols' in kwargs:
...
class RoutingConfigurationArgs:
    ...
    def _configure(
        ...
        if vnet_routes is None and 'vnetRoutes' in kwargs:
...
class VirtualApplianceSkuPropertiesArgs:
    ...
    def _configure(
             ...
             opts: Optional[pulumi.ResourceOptions] = None,
             **kwargs):
        if bundled_scale_unit is None and 'bundledScaleUnit' in kwargs:
            bundled_scale_unit = kwargs['bundledScaleUnit']
        if market_place_version is None and 'marketPlaceVersion' in kwargs:
...
class VirtualHubRouteV2Args:
    ...
    def _configure(
        ...
        if next_hops is None and 'nextHops' in kwargs:
