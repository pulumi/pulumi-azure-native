...
class FirewallPolicyArgs:
    ...
    def _configure(
        ...
        if 'threatIntelWhitelist' in kwargs:
            threat_intel_whitelist = kwargs['threatIntelWhitelist']
        if 'transportSecurity' in kwargs:
...
class FirewallPolicy(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if base_policy is not None and not isinstance(base_policy, SubResourceArgs):
            ...
            if dns_settings is not None and not isinstance(dns_settings, DnsSettingsArgs):
            ...
            if explicit_proxy is not None and not isinstance(explicit_proxy, ExplicitProxyArgs):
            ...
            if sku is not None and not isinstance(sku, FirewallPolicySkuArgs):
                sku = sku or {}
                def _setter(key, value):
                    sku[key] = value
                FirewallPolicySkuArgs._configure(_setter, **sku)
            __props__.__dict__["sku"] = sku
            if snat is not None and not isinstance(snat, FirewallPolicySNATArgs):
                snat = snat or {}
                def _setter(key, value):
                    snat[key] = value
                FirewallPolicySNATArgs._configure(_setter, **snat)
            __props__.__dict__["snat"] = snat
            if sql is not None and not isinstance(sql, FirewallPolicySQLArgs):
                sql = sql or {}
                def _setter(key, value):
                    sql[key] = value
            ...
            if threat_intel_whitelist is not None and not isinstance(threat_intel_whitelist, FirewallPolicyThreatIntelWhitelistArgs):
            ...
            if transport_security is not None and not isinstance(transport_security, FirewallPolicyTransportSecurityArgs):
