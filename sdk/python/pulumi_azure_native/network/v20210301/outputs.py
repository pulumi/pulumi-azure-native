...
class CustomIpPrefixResponse(dict):
    ...
    def _configure(
             ...
             child_custom_ip_prefixes: Optional[Sequence['outputs.CustomIpPrefixResponse']] = None,
             ...
             **kwargs):
        if child_custom_ip_prefixes is None and 'childCustomIpPrefixes' in kwargs:
            child_custom_ip_prefixes = kwargs['childCustomIpPrefixes']
        if child_custom_ip_prefixes is None:
            raise TypeError("Missing 'child_custom_ip_prefixes' argument")
        if etag is None:
        ...
        if failed_reason is None:
            raise TypeError("Missing 'failed_reason' argument")
        if name is None:
        ...
        if public_ip_prefixes is None:
            raise TypeError("Missing 'public_ip_prefixes' argument")
...
class ExpressRouteConnectionResponse(dict):
    ...
    def _configure(
             _setter: Callable[[Any, Any], None],
             express_route_circuit_peering: Optional['outputs.ExpressRouteCircuitPeeringIdResponse'] = None,
             name: Optional[str] = None,
             provisioning_state: Optional[str] = None,
             authorization_key: Optional[str] = None,
             enable_internet_security: Optional[bool] = None,
...
class VnetRouteResponse(dict):
    ...
    def _configure(
             _setter: Callable[[Any, Any], None],
             bgp_connections: Optional[Sequence['outputs.SubResourceResponse']] = None,
             static_routes: Optional[Sequence['outputs.StaticRouteResponse']] = None,
             opts: Optional[pulumi.ResourceOptions] = None,
             **kwargs):
        if bgp_connections is None and 'bgpConnections' in kwargs:
            bgp_connections = kwargs['bgpConnections']
        if bgp_connections is None:
            raise TypeError("Missing 'bgp_connections' argument")
