...
class BatchAccount(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if auto_storage is not None and not isinstance(auto_storage, AutoStorageBasePropertiesArgs):
                auto_storage = auto_storage or {}
                def _setter(key, value):
                    auto_storage[key] = value
                AutoStorageBasePropertiesArgs._configure(_setter, **auto_storage)
            ...
            if identity is not None and not isinstance(identity, BatchAccountIdentityArgs):
                identity = identity or {}
                def _setter(key, value):
                    identity[key] = value
                BatchAccountIdentityArgs._configure(_setter, **identity)
            __props__.__dict__["identity"] = identity
            if key_vault_reference is not None and not isinstance(key_vault_reference, KeyVaultReferenceArgs):
                key_vault_reference = key_vault_reference or {}
                def _setter(key, value):
                    key_vault_reference[key] = value
                KeyVaultReferenceArgs._configure(_setter, **key_vault_reference)
        ...
        super(BatchAccount, __self__).__init__(
            'azure-native:batch/v20230501:BatchAccount',
            resource_name,
            __props__,
            opts)
