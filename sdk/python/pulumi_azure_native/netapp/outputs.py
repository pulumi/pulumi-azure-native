...
class ExportPolicyRuleResponseV2(dict):
    ...
    def __key_warning(key: str):
        ...
        if suggest:
            pulumi.log.warn(f"Key '{key}' not found in ExportPolicyRuleResponseV2. Access the value via the '{suggest}' property getter instead.")

    def __getitem__(self, key: str) -> Any:
        ExportPolicyRuleResponseV2.__key_warning(key)
        return super().__getitem__(key)
...
class VolumePropertiesResponseExportPolicyV1(dict):
    ...
    """
    def __init__(__self__, *,
                 rules: Optional[Sequence['outputs.ExportPolicyRuleResponseV2']] = None):
        """
        Set of export policy rules

        :param Sequence['ExportPolicyRuleResponseV2'] rules: Export policy rule
        """
        if rules is not None:
            pulumi.set(__self__, "rules", rules)
