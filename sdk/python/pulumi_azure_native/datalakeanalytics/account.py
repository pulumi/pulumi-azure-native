...
class AccountArgs:
    ...
    def _configure(
             ...
             data_lake_store_accounts: Optional[pulumi.Input[Sequence[pulumi.Input['AddDataLakeStoreWithAccountParametersArgs']]]] = None,
             ...
             **kwargs):
        if data_lake_store_accounts is None and 'dataLakeStoreAccounts' in kwargs:
            data_lake_store_accounts = kwargs['dataLakeStoreAccounts']
        if data_lake_store_accounts is None:
            raise TypeError("Missing 'data_lake_store_accounts' argument")
        if default_data_lake_store_account is None and 'defaultDataLakeStoreAccount' in kwargs:
            default_data_lake_store_account = kwargs['defaultDataLakeStoreAccount']
        if default_data_lake_store_account is None:
            raise TypeError("Missing 'default_data_lake_store_account' argument")
        ...
        if compute_policies is None and 'computePolicies' in kwargs:
            compute_policies = kwargs['computePolicies']
        if firewall_allow_azure_ips is None and 'firewallAllowAzureIps' in kwargs:
        ...
        if firewall_state is None and 'firewallState' in kwargs:
            firewall_state = kwargs['firewallState']
        if max_degree_of_parallelism is None and 'maxDegreeOfParallelism' in kwargs:
            max_degree_of_parallelism = kwargs['maxDegreeOfParallelism']
        ...
        if new_tier is None and 'newTier' in kwargs:
            new_tier = kwargs['newTier']
        if query_store_retention is None and 'queryStoreRetention' in kwargs:
            query_store_retention = kwargs['queryStoreRetention']
