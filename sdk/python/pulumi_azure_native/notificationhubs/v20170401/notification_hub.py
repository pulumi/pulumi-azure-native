...
class NotificationHubArgs:
    ...
    def _configure(
        ...
        if 'authorizationRules' in kwargs:
...
class NotificationHub(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if adm_credential is not None and not isinstance(adm_credential, AdmCredentialArgs):
                adm_credential = adm_credential or {}
                def _setter(key, value):
                    adm_credential[key] = value
                AdmCredentialArgs._configure(_setter, **adm_credential)
            __props__.__dict__["adm_credential"] = adm_credential
            if apns_credential is not None and not isinstance(apns_credential, ApnsCredentialArgs):
                apns_credential = apns_credential or {}
                def _setter(key, value):
                    apns_credential[key] = value
                ApnsCredentialArgs._configure(_setter, **apns_credential)
            __props__.__dict__["apns_credential"] = apns_credential
            __props__.__dict__["authorization_rules"] = authorization_rules
            if baidu_credential is not None and not isinstance(baidu_credential, BaiduCredentialArgs):
                baidu_credential = baidu_credential or {}
                def _setter(key, value):
                    baidu_credential[key] = value
                BaiduCredentialArgs._configure(_setter, **baidu_credential)
            __props__.__dict__["baidu_credential"] = baidu_credential
            if gcm_credential is not None and not isinstance(gcm_credential, GcmCredentialArgs):
                gcm_credential = gcm_credential or {}
                def _setter(key, value):
                    gcm_credential[key] = value
                GcmCredentialArgs._configure(_setter, **gcm_credential)
            __props__.__dict__["gcm_credential"] = gcm_credential
            __props__.__dict__["location"] = location
            if mpns_credential is not None and not isinstance(mpns_credential, MpnsCredentialArgs):
                mpns_credential = mpns_credential or {}
                def _setter(key, value):
                    mpns_credential[key] = value
                MpnsCredentialArgs._configure(_setter, **mpns_credential)
            ...
            if wns_credential is not None and not isinstance(wns_credential, WnsCredentialArgs):
                wns_credential = wns_credential or {}
                def _setter(key, value):
                    wns_credential[key] = value
                WnsCredentialArgs._configure(_setter, **wns_credential)
