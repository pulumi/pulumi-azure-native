...
class SyncGroupArgs:
    ...
    def _configure(
        ...
        if conflict_logging_retention_in_days is None and 'conflictLoggingRetentionInDays' in kwargs:
            conflict_logging_retention_in_days = kwargs['conflictLoggingRetentionInDays']
        ...
        if enable_conflict_logging is None and 'enableConflictLogging' in kwargs:
            enable_conflict_logging = kwargs['enableConflictLogging']
        if hub_database_password is None and 'hubDatabasePassword' in kwargs:
            hub_database_password = kwargs['hubDatabasePassword']
        if hub_database_user_name is None and 'hubDatabaseUserName' in kwargs:
            hub_database_user_name = kwargs['hubDatabaseUserName']
...
class SyncGroup(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            schema = _utilities.configure(schema, SyncGroupSchemaArgs, True)
        ...
        super(SyncGroup, __self__).__init__(
            'azure-native:sql/v20230201preview:SyncGroup',
            resource_name,
            __props__,
            opts)
