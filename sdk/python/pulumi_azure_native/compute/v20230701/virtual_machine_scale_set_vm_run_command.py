...
class VirtualMachineScaleSetVMRunCommandArgs:
    def __init__(__self__, *,
        ...
        """
        VirtualMachineScaleSetVMRunCommandArgs._configure(
    ...
    def _configure(
        ...
        if vm_scale_set_name is None:
            raise TypeError("Missing 'vm_scale_set_name' argument")
        if async_execution is None and 'asyncExecution' in kwargs:
            async_execution = kwargs['asyncExecution']
        if error_blob_managed_identity is None and 'errorBlobManagedIdentity' in kwargs:
            error_blob_managed_identity = kwargs['errorBlobManagedIdentity']
        if error_blob_uri is None and 'errorBlobUri' in kwargs:
            error_blob_uri = kwargs['errorBlobUri']
        if output_blob_managed_identity is None and 'outputBlobManagedIdentity' in kwargs:
            output_blob_managed_identity = kwargs['outputBlobManagedIdentity']
        if output_blob_uri is None and 'outputBlobUri' in kwargs:
            output_blob_uri = kwargs['outputBlobUri']
        if protected_parameters is None and 'protectedParameters' in kwargs:
            protected_parameters = kwargs['protectedParameters']
        if run_as_password is None and 'runAsPassword' in kwargs:
            run_as_password = kwargs['runAsPassword']
        if run_as_user is None and 'runAsUser' in kwargs:
            run_as_user = kwargs['runAsUser']
        if run_command_name is None and 'runCommandName' in kwargs:
            run_command_name = kwargs['runCommandName']
...
class VirtualMachineScaleSetVMRunCommand(pulumi.CustomResource):
    ...
    def __init__(__self__, resource_name: str, *args, **kwargs):
        ...
        if resource_args is not None:
            ...
            def _setter(key, value):
                kwargs[key] = value
            VirtualMachineScaleSetVMRunCommandArgs._configure(_setter, **kwargs)
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            error_blob_managed_identity = _utilities.configure(error_blob_managed_identity, RunCommandManagedIdentityArgs, True)
            ...
            output_blob_managed_identity = _utilities.configure(output_blob_managed_identity, RunCommandManagedIdentityArgs, True)
            ...
            source = _utilities.configure(source, VirtualMachineRunCommandScriptSourceArgs, True)
