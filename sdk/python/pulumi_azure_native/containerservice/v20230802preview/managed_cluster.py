...
class ManagedClusterArgs:
    ...
    def _configure(
        ...
        if 'enableNamespaceResources' in kwargs:
        ...
        if 'guardrailsProfile' in kwargs:
        ...
        if 'nodeResourceGroupProfile' in kwargs:
...
class ManagedCluster(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if guardrails_profile is not None and not isinstance(guardrails_profile, GuardrailsProfileArgs):
                guardrails_profile = guardrails_profile or {}
                def _setter(key, value):
                    guardrails_profile[key] = value
                GuardrailsProfileArgs._configure(_setter, **guardrails_profile)
            ...
            if ingress_profile is not None and not isinstance(ingress_profile, ManagedClusterIngressProfileArgs):
                ingress_profile = ingress_profile or {}
                def _setter(key, value):
                    ingress_profile[key] = value
                ManagedClusterIngressProfileArgs._configure(_setter, **ingress_profile)
            ...
            if metrics_profile is not None and not isinstance(metrics_profile, ManagedClusterMetricsProfileArgs):
                metrics_profile = metrics_profile or {}
                def _setter(key, value):
                    metrics_profile[key] = value
                ManagedClusterMetricsProfileArgs._configure(_setter, **metrics_profile)
            ...
            if node_resource_group_profile is not None and not isinstance(node_resource_group_profile, ManagedClusterNodeResourceGroupProfileArgs):
                node_resource_group_profile = node_resource_group_profile or {}
                def _setter(key, value):
                    node_resource_group_profile[key] = value
                ManagedClusterNodeResourceGroupProfileArgs._configure(_setter, **node_resource_group_profile)
            ...
            if service_mesh_profile is not None and not isinstance(service_mesh_profile, ServiceMeshProfileArgs):
                service_mesh_profile = service_mesh_profile or {}
                def _setter(key, value):
                    service_mesh_profile[key] = value
                ServiceMeshProfileArgs._configure(_setter, **service_mesh_profile)
            ...
            if upgrade_settings is not None and not isinstance(upgrade_settings, ClusterUpgradeSettingsArgs):
                upgrade_settings = upgrade_settings or {}
                def _setter(key, value):
                    upgrade_settings[key] = value
                ClusterUpgradeSettingsArgs._configure(_setter, **upgrade_settings)
