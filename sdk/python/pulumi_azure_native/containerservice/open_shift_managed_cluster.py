...
class OpenShiftManagedClusterArgs:
    ...
    def _configure(
             ...
             **kwargs):
        if open_shift_version is None and 'openShiftVersion' in kwargs:
            open_shift_version = kwargs['openShiftVersion']
        if open_shift_version is None:
            raise TypeError("Missing 'open_shift_version' argument")
        ...
        if auth_profile is None and 'authProfile' in kwargs:
            auth_profile = kwargs['authProfile']
        if master_pool_profile is None and 'masterPoolProfile' in kwargs:
            master_pool_profile = kwargs['masterPoolProfile']
        if monitor_profile is None and 'monitorProfile' in kwargs:
            monitor_profile = kwargs['monitorProfile']
        if network_profile is None and 'networkProfile' in kwargs:
            network_profile = kwargs['networkProfile']
        if refresh_cluster is None and 'refreshCluster' in kwargs:
            refresh_cluster = kwargs['refreshCluster']
        if resource_name is None and 'resourceName' in kwargs:
            resource_name = kwargs['resourceName']
        if router_profiles is None and 'routerProfiles' in kwargs:
            router_profiles = kwargs['routerProfiles']
...
class OpenShiftManagedCluster(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            auth_profile = _utilities.configure(auth_profile, OpenShiftManagedClusterAuthProfileArgs, True)
            __props__.__dict__["auth_profile"] = auth_profile
            __props__.__dict__["location"] = location
            master_pool_profile = _utilities.configure(master_pool_profile, OpenShiftManagedClusterMasterPoolProfileArgs, True)
            __props__.__dict__["master_pool_profile"] = master_pool_profile
            monitor_profile = _utilities.configure(monitor_profile, OpenShiftManagedClusterMonitorProfileArgs, True)
            ...
            plan = _utilities.configure(plan, PurchasePlanArgs, True)
