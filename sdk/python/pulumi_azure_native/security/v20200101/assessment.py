...
class Assessment(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if not isinstance(metadata, SecurityAssessmentMetadataPropertiesArgs):
                metadata = metadata or {}
                def _setter(key, value):
                    metadata[key] = value
                SecurityAssessmentMetadataPropertiesArgs._configure(_setter, **metadata)
            __props__.__dict__["metadata"] = metadata
            if not isinstance(partners_data, SecurityAssessmentPartnerDataArgs):
            ...
            if not isinstance(status, AssessmentStatusArgs):
                status = status or {}
                def _setter(key, value):
                    status[key] = value
