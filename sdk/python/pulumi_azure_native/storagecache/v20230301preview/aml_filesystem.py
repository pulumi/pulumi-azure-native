...
class AmlFilesystemArgs:
    ...
    def _configure(
        ...
        if 'storageCapacityTiB' in kwargs:
            storage_capacity_ti_b = kwargs['storageCapacityTiB']
        if 'amlFilesystemName' in kwargs:
...
class AmlFilesystem(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if encryption_settings is not None and not isinstance(encryption_settings, AmlFilesystemEncryptionSettingsArgs):
                encryption_settings = encryption_settings or {}
                def _setter(key, value):
                    encryption_settings[key] = value
                AmlFilesystemEncryptionSettingsArgs._configure(_setter, **encryption_settings)
            ...
            if hsm is not None and not isinstance(hsm, AmlFilesystemHsmArgs):
                hsm = hsm or {}
                def _setter(key, value):
                    hsm[key] = value
                AmlFilesystemHsmArgs._configure(_setter, **hsm)
            __props__.__dict__["hsm"] = hsm
            if identity is not None and not isinstance(identity, AmlFilesystemIdentityArgs):
                identity = identity or {}
                def _setter(key, value):
                    identity[key] = value
                AmlFilesystemIdentityArgs._configure(_setter, **identity)
            __props__.__dict__["identity"] = identity
            __props__.__dict__["location"] = location
            if maintenance_window is not None and not isinstance(maintenance_window, AmlFilesystemMaintenanceWindowArgs):
                maintenance_window = maintenance_window or {}
                def _setter(key, value):
                    maintenance_window[key] = value
                AmlFilesystemMaintenanceWindowArgs._configure(_setter, **maintenance_window)
            ...
            if sku is not None and not isinstance(sku, SkuNameArgs):
                sku = sku or {}
                def _setter(key, value):
                    sku[key] = value
                SkuNameArgs._configure(_setter, **sku)
