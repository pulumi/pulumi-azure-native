...
class User(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if identity is not None and not isinstance(identity, UserIdentityArgs):
                identity = identity or {}
                def _setter(key, value):
                    identity[key] = value
                UserIdentityArgs._configure(_setter, **identity)
            ...
            if secret_store is not None and not isinstance(secret_store, UserSecretStoreArgs):
                secret_store = secret_store or {}
                def _setter(key, value):
                    secret_store[key] = value
                UserSecretStoreArgs._configure(_setter, **secret_store)
