...
class IdentityProviderArgs:
    ...
    def _configure(
        ...
        if client_library is None and 'clientLibrary' in kwargs:
            client_library = kwargs['clientLibrary']
        if identity_provider_name is None and 'identityProviderName' in kwargs:
            identity_provider_name = kwargs['identityProviderName']
        if password_reset_policy_name is None and 'passwordResetPolicyName' in kwargs:
            password_reset_policy_name = kwargs['passwordResetPolicyName']
        if profile_editing_policy_name is None and 'profileEditingPolicyName' in kwargs:
            profile_editing_policy_name = kwargs['profileEditingPolicyName']
        if signin_policy_name is None and 'signinPolicyName' in kwargs:
            signin_policy_name = kwargs['signinPolicyName']
        if signin_tenant is None and 'signinTenant' in kwargs:
            signin_tenant = kwargs['signinTenant']
        if signup_policy_name is None and 'signupPolicyName' in kwargs:
            signup_policy_name = kwargs['signupPolicyName']
        ...
        if client_library is not None:
            _setter("client_library", client_library)
        ...
        if password_reset_policy_name is not None:
            _setter("password_reset_policy_name", password_reset_policy_name)
        if profile_editing_policy_name is not None:
            _setter("profile_editing_policy_name", profile_editing_policy_name)
        if signin_policy_name is not None:
            _setter("signin_policy_name", signin_policy_name)
        if signin_tenant is not None:
            _setter("signin_tenant", signin_tenant)
        if signup_policy_name is not None:
            _setter("signup_policy_name", signup_policy_name)
        if type is not None:
            _setter("type", type)
...
class IdentityProvider(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        super(IdentityProvider, __self__).__init__(
            'azure-native:apimanagement/v20230301preview:IdentityProvider',
            resource_name,
            __props__,
            opts)
