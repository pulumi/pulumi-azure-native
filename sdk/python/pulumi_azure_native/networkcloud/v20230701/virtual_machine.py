...
class VirtualMachineArgs:
    ...
    def _configure(
             ...
             cloud_services_network_attachment: Optional[pulumi.Input['NetworkAttachmentArgs']] = None,
        ...
        if cloud_services_network_attachment is None and 'cloudServicesNetworkAttachment' in kwargs:
            cloud_services_network_attachment = kwargs['cloudServicesNetworkAttachment']
        if cloud_services_network_attachment is None:
            raise TypeError("Missing 'cloud_services_network_attachment' argument")
        ...
        if boot_method is None and 'bootMethod' in kwargs:
            boot_method = kwargs['bootMethod']
        if isolate_emulator_thread is None and 'isolateEmulatorThread' in kwargs:
            isolate_emulator_thread = kwargs['isolateEmulatorThread']
        ...
        if placement_hints is None and 'placementHints' in kwargs:
            placement_hints = kwargs['placementHints']
        ...
        if virtio_interface is None and 'virtioInterface' in kwargs:
            virtio_interface = kwargs['virtioInterface']
        if virtual_machine_name is None and 'virtualMachineName' in kwargs:
            virtual_machine_name = kwargs['virtualMachineName']
        if vm_device_model is None and 'vmDeviceModel' in kwargs:
            vm_device_model = kwargs['vmDeviceModel']
        if vm_image_repository_credentials is None and 'vmImageRepositoryCredentials' in kwargs:
            vm_image_repository_credentials = kwargs['vmImageRepositoryCredentials']
...
class VirtualMachine(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            cloud_services_network_attachment = _utilities.configure(cloud_services_network_attachment, NetworkAttachmentArgs, True)
            ...
            vm_image_repository_credentials = _utilities.configure(vm_image_repository_credentials, ImageRepositoryCredentialsArgs, True)
        ...
        alias_opts = pulumi.ResourceOptions(aliases=[pulumi.Alias(type_="azure-native:networkcloud:VirtualMachine"), pulumi.Alias(type_="azure-native:networkcloud/v20221212preview:VirtualMachine"), pulumi.Alias(type_="azure-native:networkcloud/v20230501preview:VirtualMachine")])
        opts = pulumi.ResourceOptions.merge(opts, alias_opts)
        super(VirtualMachine, __self__).__init__(
            'azure-native:networkcloud/v20230701:VirtualMachine',
            resource_name,
            __props__,
            opts)
