...
class StreamingJob(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if not isinstance(cluster, ClusterInfoArgs):
                cluster = cluster or {}
                def _setter(key, value):
                    cluster[key] = value
            ...
            if not isinstance(externals, ExternalArgs):
                externals = externals or {}
                def _setter(key, value):
                    externals[key] = value
                ExternalArgs._configure(_setter, **externals)
            ...
            if not isinstance(job_storage_account, JobStorageAccountArgs):
            ...
            if not isinstance(sku, StreamingJobSkuArgs):
                sku = sku or {}
                def _setter(key, value):
                    sku[key] = value
                StreamingJobSkuArgs._configure(_setter, **sku)
            __props__.__dict__["sku"] = sku
            __props__.__dict__["tags"] = tags
            if not isinstance(transformation, TransformationArgs):
