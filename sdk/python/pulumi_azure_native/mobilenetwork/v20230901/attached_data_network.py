...
class AttachedDataNetwork(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if napt_configuration is not None and not isinstance(napt_configuration, NaptConfigurationArgs):
                napt_configuration = napt_configuration or {}
                def _setter(key, value):
                    napt_configuration[key] = value
                NaptConfigurationArgs._configure(_setter, **napt_configuration)
            ...
            if user_plane_data_interface is not None and not isinstance(user_plane_data_interface, InterfacePropertiesArgs):
                user_plane_data_interface = user_plane_data_interface or {}
                def _setter(key, value):
                    user_plane_data_interface[key] = value
                InterfacePropertiesArgs._configure(_setter, **user_plane_data_interface)
