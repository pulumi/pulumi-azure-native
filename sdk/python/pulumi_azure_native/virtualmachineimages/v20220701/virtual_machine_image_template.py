...
class VirtualMachineImageTemplateArgs:
    ...
    def _configure(
        ...
        if 'buildTimeoutInMinutes' in kwargs:
            build_timeout_in_minutes = kwargs['buildTimeoutInMinutes']
        if 'imageTemplateName' in kwargs:
            image_template_name = kwargs['imageTemplateName']
        if 'stagingResourceGroup' in kwargs:
...
class VirtualMachineImageTemplate(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            if identity is not None and not isinstance(identity, ImageTemplateIdentityArgs):
                identity = identity or {}
                def _setter(key, value):
                    identity[key] = value
                ImageTemplateIdentityArgs._configure(_setter, **identity)
            if identity is None and not opts.urn:
                raise TypeError("Missing required property 'identity'")
            __props__.__dict__["identity"] = identity
            __props__.__dict__["image_template_name"] = image_template_name
            __props__.__dict__["location"] = location
            if optimize is not None and not isinstance(optimize, ImageTemplatePropertiesOptimizeArgs):
                optimize = optimize or {}
                def _setter(key, value):
                    optimize[key] = value
                ImageTemplatePropertiesOptimizeArgs._configure(_setter, **optimize)
            ...
            if validate is not None and not isinstance(validate, ImageTemplatePropertiesValidateArgs):
                validate = validate or {}
                def _setter(key, value):
                    validate[key] = value
                ImageTemplatePropertiesValidateArgs._configure(_setter, **validate)
            __props__.__dict__["validate"] = validate
            if vm_profile is not None and not isinstance(vm_profile, ImageTemplateVmProfileArgs):
                vm_profile = vm_profile or {}
                def _setter(key, value):
                    vm_profile[key] = value
                ImageTemplateVmProfileArgs._configure(_setter, **vm_profile)
