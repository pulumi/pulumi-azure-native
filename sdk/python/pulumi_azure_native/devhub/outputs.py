...
class WorkflowRunResponse(dict):
    ...
    def _configure(
             ...
             opts: Optional[pulumi.ResourceOptions] = None,
             **kwargs):
        if last_run_at is None and 'lastRunAt' in kwargs:
            last_run_at = kwargs['lastRunAt']
        if last_run_at is None:
            raise TypeError("Missing 'last_run_at' argument")
        if succeeded is None:
            raise TypeError("Missing 'succeeded' argument")
        if workflow_run_url is None and 'workflowRunURL' in kwargs:
            workflow_run_url = kwargs['workflowRunURL']
        if workflow_run_url is None:
            raise TypeError("Missing 'workflow_run_url' argument")
