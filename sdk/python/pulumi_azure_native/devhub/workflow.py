...
class WorkflowArgs:
    ...
    def _configure(
        ...
        if aks_resource_id is None and 'aksResourceId' in kwargs:
            aks_resource_id = kwargs['aksResourceId']
        ...
        if docker_build_context is None and 'dockerBuildContext' in kwargs:
            docker_build_context = kwargs['dockerBuildContext']
        if dockerfile_generation_mode is None and 'dockerfileGenerationMode' in kwargs:
            dockerfile_generation_mode = kwargs['dockerfileGenerationMode']
        if dockerfile_output_directory is None and 'dockerfileOutputDirectory' in kwargs:
            dockerfile_output_directory = kwargs['dockerfileOutputDirectory']
        if generation_language is None and 'generationLanguage' in kwargs:
            generation_language = kwargs['generationLanguage']
        ...
        if last_workflow_run is None and 'lastWorkflowRun' in kwargs:
            last_workflow_run = kwargs['lastWorkflowRun']
        if manifest_generation_mode is None and 'manifestGenerationMode' in kwargs:
            manifest_generation_mode = kwargs['manifestGenerationMode']
        if manifest_output_directory is None and 'manifestOutputDirectory' in kwargs:
            manifest_output_directory = kwargs['manifestOutputDirectory']
        if manifest_type is None and 'manifestType' in kwargs:
            manifest_type = kwargs['manifestType']
        if oidc_credentials is None and 'oidcCredentials' in kwargs:
            oidc_credentials = kwargs['oidcCredentials']
...
class Workflow(pulumi.CustomResource):
    ...
    def _internal_init(__self__,
        ...
        if opts.id is None:
            ...
            acr = _utilities.configure(acr, ACRArgs, True)
            ...
            deployment_properties = _utilities.configure(deployment_properties, DeploymentPropertiesArgs, True)
            ...
            last_workflow_run = _utilities.configure(last_workflow_run, WorkflowRunArgs, True)
            ...
            oidc_credentials = _utilities.configure(oidc_credentials, GitHubWorkflowProfileOidcCredentialsArgs, True)
        ...
        alias_opts = pulumi.ResourceOptions(aliases=[pulumi.Alias(type_="azure-native:devhub/v20220401preview:Workflow"), pulumi.Alias(type_="azure-native:devhub/v20221011preview:Workflow"), pulumi.Alias(type_="azure-native:devhub/v20230801:Workflow")])
