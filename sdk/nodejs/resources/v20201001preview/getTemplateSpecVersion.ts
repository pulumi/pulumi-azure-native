// *** WARNING: this file was generated by the Pulumi SDK Generator. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as inputs from "../../types/input";
import * as outputs from "../../types/output";
import * as utilities from "../../utilities";

export function getTemplateSpecVersion(args: GetTemplateSpecVersionArgs, opts?: pulumi.InvokeOptions): Promise<GetTemplateSpecVersionResult> {
    if (!opts) {
        opts = {}
    }

    if (!opts.version) {
        opts.version = utilities.getVersion();
    }
    return pulumi.runtime.invoke("azure-nextgen:resources/v20201001preview:getTemplateSpecVersion", {
        "resourceGroupName": args.resourceGroupName,
        "templateSpecName": args.templateSpecName,
        "templateSpecVersion": args.templateSpecVersion,
    }, opts);
}

export interface GetTemplateSpecVersionArgs {
    /**
     * The name of the resource group. The name is case insensitive.
     */
    readonly resourceGroupName: string;
    /**
     * Name of the Template Spec.
     */
    readonly templateSpecName: string;
    /**
     * The version of the Template Spec.
     */
    readonly templateSpecVersion: string;
}

/**
 * Template Spec Version object.
 */
export interface GetTemplateSpecVersionResult {
    /**
     * An array of Template Spec artifacts.
     */
    readonly artifacts?: outputs.resources.v20201001preview.TemplateSpecTemplateArtifactResponse[];
    /**
     * Template Spec version description.
     */
    readonly description?: string;
    /**
     * The location of the Template Spec Version. It must match the location of the parent Template Spec.
     */
    readonly location: string;
    /**
     * Name of this resource.
     */
    readonly name: string;
    /**
     * Azure Resource Manager metadata containing createdBy and modifiedBy information.
     */
    readonly systemData: outputs.resources.v20201001preview.SystemDataResponse;
    /**
     * Resource tags.
     */
    readonly tags?: {[key: string]: string};
    /**
     * The Azure Resource Manager template content.
     */
    readonly template?: any;
    /**
     * Type of this resource.
     */
    readonly type: string;
    /**
     * The Azure Resource Manager template UI definition content
     */
    readonly uiDefinition?: any;
}
