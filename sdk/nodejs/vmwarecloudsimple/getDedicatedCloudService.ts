// *** WARNING: this file was generated by the Pulumi SDK Generator. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

import * as pulumi from "@pulumi/pulumi";
import * as inputs from "../types/input";
import * as outputs from "../types/output";
import * as utilities from "../utilities";

export function getDedicatedCloudService(args: GetDedicatedCloudServiceArgs, opts?: pulumi.InvokeOptions): Promise<GetDedicatedCloudServiceResult> {
    if (!opts) {
        opts = {}
    }

    if (!opts.version) {
        opts.version = utilities.getVersion();
    }
    return pulumi.runtime.invoke("azurerm:vmwarecloudsimple:getDedicatedCloudService", {
        "name": args.name,
        "resourceGroupName": args.resourceGroupName,
    }, opts);
}

export interface GetDedicatedCloudServiceArgs {
    /**
     * dedicated cloud Service name
     */
    readonly name: string;
    /**
     * The name of the resource group
     */
    readonly resourceGroupName: string;
}

/**
 * Dedicated cloud service model
 */
export interface GetDedicatedCloudServiceResult {
    /**
     * Azure region
     */
    readonly location: string;
    /**
     * {dedicatedCloudServiceName}
     */
    readonly name: string;
    /**
     * The properties of Dedicated Node Service
     */
    readonly properties: outputs.vmwarecloudsimple.DedicatedCloudServicePropertiesResponse;
    /**
     * The list of tags
     */
    readonly tags?: outputs.vmwarecloudsimple.TagsResponse;
    /**
     * {resourceProviderNamespace}/{resourceType}
     */
    readonly type: string;
}
