...
func newStorageAccountStaticWebsite_azidentity(env *azure.Environment, saUrl azblob.ServiceURL) *CustomResource {
...
func createStorageAccountURL(pipeline pipeline.Pipeline, saName string) (azblob.ServiceURL, error) {
	// TODO env
	primaryEndpoint, err := neturl.Parse(fmt.Sprintf("https://%s.blob.core.windows.net", saName))
	if err != nil {
		return azblob.ServiceURL{}, err
	}
	return azblob.NewServiceURL(*primaryEndpoint, pipeline), nil
}
...
type staticWebsite_azidentity struct {
	// The service URL for the storage account
	saUrl azblob.ServiceURL
...
func is404StorageError(err error) bool {
	...
	return ok && stgErr.Response().StatusCode == http.StatusNotFound
}
...
func (r *staticWebsite_azidentity) read(ctx context.Context, id string, properties resource.PropertyMap) (map[string]interface{}, bool, error) {
	accountProps, err := r.saUrl.GetProperties(ctx)
	if err != nil {
		// stgErr.ServiceCode has more specific error conditions but none for "account not found".
	...
	if accountProps.StaticWebsite == nil || !accountProps.StaticWebsite.Enabled {
...
func (r *staticWebsite_azidentity) delete(ctx context.Context, id string, properties resource.PropertyMap) error {
	...
	resp, err := r.saUrl.SetProperties(ctx, azblob.StorageServiceProperties{
		StaticWebsite: &azblob.StaticWebsite{
			Enabled: false,
		},
	})
	...
	if err != nil {
		...
		if is404StorageError(err) || (resp != nil && resp.StatusCode() == http.StatusNotFound) {
			return nil
		}
