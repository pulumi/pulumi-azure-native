...
func main() {
	...
	supportedOutputs := codegen.NewStringSet("schema", "docs", "go", "schema-v2")
	...
	if languageSet.Has("schema") {
		...
		versions := openapi.ApplyProvidersTransformations(providers, versionMetadata.V1Lock, nil, versionMetadata.Deprecated, removed)
